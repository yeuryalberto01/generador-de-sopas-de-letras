    ...
    <div className="bg-primary rounded-xl shadow-xl border border-primary p-4 sm:p-6 h-full flex flex-col smooth-transition">
      ...
      <div className="flex flex-col gap-4 flex-1">
        ...
        <div className="flex-1 flex flex-col">
          <label className="block text-sm font-medium text-secondary mb-1">Palabras (lote)</label>
          <textarea
            value={batchInput}
            ...
            className="flex-1 px-3 py-2 border border-secondary bg-primary text-primary rounded-lg focus:outline-none focus:ring-2 focus:ring-accent resize-none font-mono text-sm"
          />
        ...
        {preview && (
          <div className="bg-accent/5 border border-accent/20 rounded-lg p-3">
            <p className="text-sm font-medium text-accent mb-2">{preview.words.length} palabras detectadas</p>
            {preview.duplicates > 0 && <p className="text-xs text-accent mb-1">{preview.duplicates} duplicados ignorados</p>}
            ...
            <div className="flex flex-wrap gap-1 mt-2">
              {preview.words.slice(0, 20).map((word, idx) => (
                <span key={idx} className="bg-primary px-2 py-1 rounded text-xs text-secondary border border-accent/20">{word}</span>
              ))}
              {preview.words.length > 20 && <span className="text-xs text-accent px-2 py-1">+{preview.words.length - 20} más...</span>}
            </div>
          </div>
        )}
        {error && (
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            className="w-full bg-accent text-white px-4 py-3 rounded-lg font-medium hover:bg-accent-hover disabled:bg-disabled disabled:cursor-not-allowed smooth-transition flex items-center justify-center gap-2 text-base"
          >
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 px-4 py-2 border border-secondary rounded-lg font-medium hover:bg-secondary text-primary disabled:opacity-50 smooth-transition"
            >
              Previsualizar
            </button>
            <button
